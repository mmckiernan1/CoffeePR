...
export function GuidedPayrollRun({ approved, timeReady, employees, gross, net, remittance, fee, onHome, onOpenEmployees, onOpenTime, onOpenReview, onApprove, onOpenPayments, onOpenReports, runKey = "run-17" }: GuidedPayrollRunProps) {
  ...
  useEffect(() => {
    if (!hydrated) return;
    try { window.sessionStorage.setItem(storageKey, JSON.stringify({ step, changesConfirmed, employeesConfirmed })); } catch { /* navigation remains usable */ }
  }, [step, changesConfirmed, employeesConfirmed, hydrated, storageKey]);

  function saveProgress(progress: SavedProgress) {
    try { window.sessionStorage.setItem(storageKey, JSON.stringify(progress)); } catch { /* navigation remains usable */ }
  }
  ...
  return (
    <RunPayrollShell currentStep={step} completedThrough={completedThrough} onStepChange={go} onBack={() => go(step - 1)} onHome={onHome} title={step === 0 ? "Anything changed?" : undefined} detail={step === 0 ? "Tell Coffee Payroll what is different this pay period. If nothing changed, you can keep moving." : undefined}>
      {step === 0 && (
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <button type="button" onClick={openEmployeeChanges} className="group rounded-2xl border border-[#c9d5e6] bg-white p-5 text-left transition hover:border-[#8fb0e8] hover:bg-[#f7f9fd]">
            <span className="grid size-11 place-items-center rounded-xl bg-[#edf3ff] text-[#1557d8]"><Users className="size-5" /></span><h2 className="mt-4 text-lg font-semibold text-[#172033]">Yes, something changed</h2><p className="mt-1 text-sm leading-6 text-[#647087]">New hire, leave, termination, pay-rate change, bonus or another employee change.</p><span className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-[#1557d8]">Review changes <ChevronRight className="size-4 transition group-hover:translate-x-0.5" /></span>
          </button>
        </div>
      )}
      ...
      {step === 1 && (
        <div>
          <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
            <div><p className="text-[11px] font-bold uppercase tracking-[0.15em] text-[#647087]">Step 2 · Employees</p><h2 className="mt-1 text-2xl font-semibold text-[#172033]">Here&apos;s who we&apos;re paying this time</h2><p className="mt-1 max-w-2xl text-sm leading-6 text-[#647087]">Coffee Payroll carried forward your regular employee list. Check the people below, especially anyone who is new or leaving.</p></div>
          ...
          {attentionEmployees.length > 0 && <div className="mb-4 rounded-xl border border-[#ead1b6] bg-[#fff8ee] px-4 py-3 text-sm text-[#755032]"><strong>{attentionEmployees.length} employee {attentionEmployees.length === 1 ? "change needs" : "changes need"} a quick look.</strong> The rest of the employee list is carried forward normally.</div>}
          <div className="grid gap-3 md:grid-cols-2">
            {employees.map((employee) => {
              const cue = employeeCue(employee);
              const isHourly = employee.payType.toLowerCase() === "hourly";
              const tileClass = cue.needsAttention ? "border-[#ddbf9f] bg-[#fffaf4] hover:border-[#c58d58] hover:shadow-sm" : cue.isNew ? "border-[#c9d9ef] bg-[#f8fbff]" : "border-[#dce4f0] bg-white";
              const content = <><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><strong className="text-sm text-[#172033]">{employee.name}</strong>{isHourly && <Badge className="border border-[#eadfbd] bg-[#fff8e7] text-[#725a22]">Hourly</Badge>}{cue.isNew && <Badge className="border-0 bg-[#eaf2ff] text-[#225caa]"><UserPlus className="mr-1 size-3" />New hire</Badge>}{cue.isLeaving && <Badge className="border-0 bg-[#fff0e5] text-[#9a5127]">Leaving</Badge>}</div><p className="mt-2 text-xs leading-5 text-[#647087]">{employee.detail}</p></div>{!cue.needsAttention && <Badge className="shrink-0 border-0 bg-[#eef9e8] text-[#34701d]">Included</Badge>}</div>{cue.changeLabel && <div className={`mt-3 flex items-center justify-between gap-2 rounded-lg px-3 py-2 text-xs font-semibold ${cue.needsAttention ? "bg-[#fff0dc] text-[#7a4a24]" : "bg-[#f3f6fa] text-[#53647b]"}`}><span>{cue.changeLabel}</span>{cue.needsAttention && <span className="inline-flex items-center gap-1">Review <ChevronRight className="size-3.5" /></span>}</div>}</>;
              return cue.needsAttention ? <button key={employee.id ?? employee.name} type="button" onClick={openEmployeesFromRoster} className={`rounded-2xl border p-4 text-left transition ${tileClass}`}>{content}</button> : <div key={employee.id ?? employee.name} className={`rounded-2xl border p-4 ${tileClass}`}>{content}</div>;
            })}
          </div>
      ...
      {step === 2 && <div className="grid gap-5 lg:grid-cols-[1fr_300px]"><div><div className="flex items-start gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-[#edf3ff] text-[#1557d8]"><Clock3 className="size-5" /></span><div><h2 className="text-xl font-semibold text-[#172033]">Enter this pay</h2><p className="mt-1 text-sm leading-6 text-[#647087]">Salaried employees carry forward automatically. Enter hours or one-time pay only where needed.</p></div></div><div className="mt-5 space-y-3">{hourlyEmployees.map((employee) => <button key={employee.id ?? employee.name} type="button" onClick={openTimeEntry} className="flex w-full items-center justify-between gap-3 rounded-xl border border-[#dce4f0] bg-white p-4 text-left transition hover:border-[#8fb0e8] hover:bg-[#f7f9fd]"><span><strong className="block text-sm">{employee.name}</strong><span className="mt-1 block text-xs text-[#647087]">{employee.detail}</span></span><span className="inline-flex items-center gap-1 text-xs font-semibold text-[#1557d8]">Enter hours <ChevronRight className="size-4" /></span></button>)}</div><div className="mt-5 flex justify-end"><Button type="button" disabled={!timeReady} onClick={() => go(3)} className="bg-[#1557d8] text-white hover:bg-[#0f47b5] disabled:bg-[#aebbd0]">{timeReady ? "Hours and pay are ready" : "Finish hours to continue"}<ChevronRight className="size-4" /></Button></div></div><aside className="rounded-2xl border border-[#dce4f0] bg-[#f7f9fd] p-4"><CalendarDays className="size-5 text-[#1557d8]" /><h3 className="mt-3 text-sm font-semibold">Coffee Payroll handles the routine</h3><p className="mt-2 text-xs leading-5 text-[#647087]">Regular salary, recurring deductions and configured statutory rules continue automatically unless something changed.</p></aside></div>}
