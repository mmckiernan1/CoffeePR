...
import { addUtcMonths, monthlyRemittanceDueDate, remittanceLiabilityCents, validateOvertimeBankMovement } from "@/lib/payroll/operations";
...
export async function POST(request: Request) {
  ...
  if (body.action === "approve_draft") {
    ...
    const remittanceDueDate = monthlyRemittanceDueDate(draft.payDate);
    ...
    await db.batch([
      ...
      db.prepare("INSERT INTO remittance_obligations (id, workspace_id, payroll_account_id, pay_run_id, period_end, due_date, liability_cents, status, reminder_date, paid_date, payment_reference, created_at, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(`REM-${payRunId}`, WORKSPACE_ID, draft.payrollAccountId, payRunId, draft.periodEnd, remittanceDueDate, remittanceAmountCents, "Due", null, null, null, occurredAt, actor.email),
