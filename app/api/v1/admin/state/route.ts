...
type AdminAction =
  ...
  | { action: "calculate_draft"; draftId: string; lines: Array<{ employeeId: string; regularHoursHundredths: number; overtimeHoursHundredths: number; bankedOvertimeEarnedHundredths: number; bankedOvertimeUsedHundredths: number; otherEarningsCents: number; otherDeductionsCents: number }> }
...
async function ensureFictionalWorkspace(actorEmail: string) {
  ...
  await db.batch([
    ...
    db.prepare("INSERT OR IGNORE INTO client_onboarding_profiles (id, workspace_id, service_path, status, updated_at, updated_by) VALUES (?, ?, ?, ?, ?, ?)").bind("COP-PNS-001", WORKSPACE_ID, "Self-service", "Ready for first payroll", createdAt, actorEmail),
...
async function state(actor: { email: string; role: ComcheqRole }) {
  ...
  const [accounts, employees, offboarding, memberships, payrollSettings, onboardingProfiles, schedules, payrollProfiles, payrollCodes, recurringPayItems, payRuns, outputs, billing, activeDrafts, draftLines, draftComponents, complianceChecks, payments, paymentComponents, overtimeAgreements, overtimeBankEntries, overtimeBalances, remittances, bankLinks, audit] = await Promise.all([
    ...
    db.prepare("SELECT id, service_path AS servicePath, status, updated_at AS updatedAt, updated_by AS updatedBy FROM client_onboarding_profiles WHERE workspace_id = ? LIMIT 1").bind(WORKSPACE_ID).all(),
    ...
    db.prepare("SELECT b.employee_id AS employeeId, e.legal_name AS employeeName, SUM(b.hours_delta_hundredths) AS balanceHundredths, MIN(CASE WHEN b.hours_delta_hundredths > 0 AND b.expires_on >= date('now') THEN b.expires_on END) AS nextExpiryDate FROM overtime_bank_entries b JOIN employees e ON e.id = b.employee_id WHERE b.workspace_id = ? GROUP BY b.employee_id, e.legal_name ORDER BY e.legal_name").bind(WORKSPACE_ID).all(),
    db.prepare("SELECT id, payroll_account_id AS payrollAccountId, pay_run_id AS payRunId, period_end AS periodEnd, due_date AS dueDate, liability_cents AS liabilityCents, status, reminder_date AS reminderDate, paid_date AS paidDate, payment_reference AS paymentReference, created_at AS createdAt FROM remittance_obligations WHERE workspace_id = ? ORDER BY due_date ASC").bind(WORKSPACE_ID).all(),
...
type CalculationProfile = { employeeId: string; employeeName: string; payType: string; taxMethod: string; salaryPeriodicCents: number; hourlyRateCents: number; federalClaimCents: number; albertaClaimCents: number; additionalTaxCents: number; cppExempt: number; eiExempt: number; ytdPensionableEarningsCents: number; ytdCppCents: number; ytdCpp2Cents: number; ytdEiCents: number; overtimeAgreementActive: boolean; overtimeBankBalanceHundredths: number };
type CalculationProfileRow = Omit<CalculationProfile, "overtimeAgreementActive" | "overtimeBankBalanceHundredths">;

function calculateDraftLine(input: { employeeId: string; regularHoursHundredths: number; overtimeHoursHundredths: number; bankedOvertimeEarnedHundredths: number; bankedOvertimeUsedHundredths: number; otherEarningsCents: number; otherDeductionsCents: number }, payDate: string, periodsRemainingIncludingCurrent: number, profile: CalculationProfile) {
  ...
  const bankedOvertimeEarnedHundredths = controlledInteger(input.bankedOvertimeEarnedHundredths, `${profile.employeeName} banked overtime earned`, 10_000);
  const bankedOvertimeUsedHundredths = controlledInteger(input.bankedOvertimeUsedHundredths, `${profile.employeeName} banked overtime used`, 10_000);
  ...
  if (regularHoursHundredths + overtimeHoursHundredths + bankedOvertimeEarnedHundredths + bankedOvertimeUsedHundredths > 12_000) exceptions.push("BLOCKING: Total hours exceed the 120-hour review limit.");
  if (overtimeHoursHundredths + bankedOvertimeEarnedHundredths > 2_400) exceptions.push("REVIEW: Overtime exceeds 24 hours.");
  if (profile.payType === "Salary" && (overtimeHoursHundredths > 0 || bankedOvertimeEarnedHundredths > 0 || bankedOvertimeUsedHundredths > 0)) exceptions.push("BLOCKING: Salary overtime requires a separately configured earning rule.");
  try {
    validateOvertimeBankMovement({ earnedHundredths: bankedOvertimeEarnedHundredths, usedHundredths: bankedOvertimeUsedHundredths, currentBalanceHundredths: profile.overtimeBankBalanceHundredths, agreementActive: profile.overtimeAgreementActive });
  } catch (problem) {
    exceptions.push(`BLOCKING: ${problem instanceof Error ? problem.message : "Overtime bank validation failed."}`);
  }
  ...
  const bankedOvertimeUsedEarningsCents = profile.payType === "Hourly" ? Math.round((profile.hourlyRateCents * bankedOvertimeUsedHundredths) / 100) : 0;
  const grossCents = regularEarningsCents + overtimeEarningsCents + bankedOvertimeUsedEarningsCents + otherEarningsCents;
  try {
    ...
    return {
      ...
      bankedOvertimeEarnedHundredths,
      bankedOvertimeUsedHundredths,
      otherEarningsCents,
      ...
      bankedOvertimeUsedEarningsCents,
    ...
    return { ...input, regularHoursHundredths, overtimeHoursHundredths, bankedOvertimeEarnedHundredths, bankedOvertimeUsedHundredths, otherEarningsCents, otherDeductionsCents, employeeName: profile.employeeName, payType: profile.payType, regularEarningsCents, overtimeEarningsCents, bankedOvertimeUsedEarningsCents, grossCents, incomeTaxCents: 0, cppCents: 0, cpp2Cents: 0, eiCents: 0, netPayCents: grossCents, exceptions };
  }
}
...
export async function POST(request: Request) {
  ...
  if (body.action === "update_onboarding_service_path") {
    ...
    if (!["Self-service", "Shoebox setup"].includes(body.servicePath)) return Response.json({ error: "Choose a supported onboarding path." }, { status: 400 });
    await db.batch([
      db.prepare("UPDATE client_onboarding_profiles SET service_path = ?, status = ?, updated_at = ?, updated_by = ? WHERE workspace_id = ?").bind(body.servicePath, "Ready for first payroll", occurredAt, actor.email, WORKSPACE_ID),
      db.prepare("INSERT INTO audit_events (id, workspace_id, occurred_at, actor_email, action, entity_type, entity_id, summary, payload_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(`AE-${crypto.randomUUID()}`, WORKSPACE_ID, occurredAt, actor.email, "onboarding.service_path_updated", "client_onboarding_profile", "COP-PNS-001", `${body.servicePath} onboarding path selected`, JSON.stringify({ servicePath: body.servicePath })),
    ]);
    return Response.json({ ok: true, servicePath: body.servicePath });
  }
  ...
  if (body.action === "calculate_draft") {
    ...
    try {
      ...
      const agreementResult = await db.prepare("SELECT employee_id AS employeeId FROM overtime_agreements WHERE workspace_id = ? AND status = 'Active' AND effective_from <= ? AND (effective_to IS NULL OR effective_to >= ?)").bind(WORKSPACE_ID, draft.payDate, draft.payDate).all<{ employeeId: string }>();
      const balanceResult = await db.prepare("SELECT employee_id AS employeeId, SUM(hours_delta_hundredths) AS balanceHundredths FROM overtime_bank_entries WHERE workspace_id = ? GROUP BY employee_id").bind(WORKSPACE_ID).all<{ employeeId: string; balanceHundredths: number }>();
      const agreementIds = new Set(agreementResult.results.map((item) => item.employeeId));
      const balanceMap = new Map(balanceResult.results.map((item) => [item.employeeId, Number(item.balanceHundredths ?? 0)]));
      const profileMap = new Map(profileResult.results.map((profile) => [profile.employeeId, { ...profile, overtimeAgreementActive: agreementIds.has(profile.employeeId), overtimeBankBalanceHundredths: balanceMap.get(profile.employeeId) ?? 0 }]));
      ...
      const components = calculated.flatMap((line) => {
        ...
        if (line.bankedOvertimeUsedEarningsCents > 0) items.push({ category: "Earning", code: "OT-BANK-USED", description: "Paid time off from overtime bank", quantityHundredths: line.bankedOvertimeUsedHundredths, rateCents: profile.hourlyRateCents, amountCents: line.bankedOvertimeUsedEarningsCents });
      ...
      const checks = [
        ...
        { code: "AB_OVERTIME_8_44", title: "Overtime 8/44 and bank review", status: hasHourlyEmployees ? "Review required" : "Not applicable", severity: hasHourlyEmployees ? "Review" : "Info", summary: hasHourlyEmployees ? "Confirm the greater-of daily/weekly overtime result and any banked hours. Banking requires a current written agreement and use within six months." : "No hourly employees are included in this run.", evidence: { hourlyEmployees: calculated.filter((line) => line.payType === "Hourly").map((line) => ({ employeeId: line.employeeId, regularHoursHundredths: line.regularHoursHundredths, overtimeHoursHundredths: line.overtimeHoursHundredths, bankedOvertimeEarnedHundredths: line.bankedOvertimeEarnedHundredths, bankedOvertimeUsedHundredths: line.bankedOvertimeUsedHundredths })), overtimeRateMultiplier: 1.5, bankRate: 1, bankingDeadlineMonths: 6 } },
      ...
      await db.batch([
        ...
        ...calculated.map((line) => db.prepare("UPDATE pay_run_draft_lines SET regular_hours_hundredths = ?, overtime_hours_hundredths = ?, banked_overtime_earned_hundredths = ?, banked_overtime_used_hundredths = ?, other_earnings_cents = ?, other_deductions_cents = ?, gross_cents = ?, income_tax_cents = ?, cpp_cents = ?, cpp2_cents = ?, ei_cents = ?, net_pay_cents = ?, exceptions_json = ?, updated_at = ? WHERE draft_id = ? AND employee_id = ? AND workspace_id = ?")
          .bind(line.regularHoursHundredths, line.overtimeHoursHundredths, line.bankedOvertimeEarnedHundredths, line.bankedOvertimeUsedHundredths, line.otherEarningsCents, line.otherDeductionsCents, line.grossCents, line.incomeTaxCents, line.cppCents, line.cpp2Cents, line.eiCents, line.netPayCents, JSON.stringify(line.exceptions), occurredAt, draft.id, line.employeeId, WORKSPACE_ID)),
  ...
  if (body.action === "schedule_remittance_reminder") {
    if (actor.role === "Read-only") return Response.json({ error: "Payroll Processor or Administrator role required." }, { status: 403 });
    const remittance = await db.prepare("SELECT id, due_date AS dueDate, status FROM remittance_obligations WHERE id = ? AND workspace_id = ? LIMIT 1").bind(body.remittanceId, WORKSPACE_ID).first<{ id: string; dueDate: string; status: string }>();
    if (!remittance) return Response.json({ error: "The remittance obligation was not found." }, { status: 404 });
    if (remittance.status === "Paid") return Response.json({ error: "A paid remittance does not require a reminder." }, { status: 409 });
    if (!/^\d{4}-\d{2}-\d{2}$/.test(body.reminderDate) || body.reminderDate > remittance.dueDate) return Response.json({ error: "Choose a reminder date on or before the due date." }, { status: 400 });
    await db.batch([
      db.prepare("UPDATE remittance_obligations SET status = ?, reminder_date = ? WHERE id = ? AND workspace_id = ?").bind("Reminder scheduled", body.reminderDate, remittance.id, WORKSPACE_ID),
      db.prepare("INSERT INTO audit_events (id, workspace_id, occurred_at, actor_email, action, entity_type, entity_id, summary, payload_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(`AE-${crypto.randomUUID()}`, WORKSPACE_ID, occurredAt, actor.email, "remittance.reminder_scheduled", "remittance", remittance.id, `CRA remittance reminder scheduled for ${body.reminderDate}`, JSON.stringify({ dueDate: remittance.dueDate, reminderDate: body.reminderDate })),
    ]);
    return Response.json({ ok: true, id: remittance.id, status: "Reminder scheduled" });
  }

  if (body.action === "mark_remittance_paid") {
    if (actor.role === "Read-only") return Response.json({ error: "Payroll Processor or Administrator role required." }, { status: 403 });
    if (!/^\d{4}-\d{2}-\d{2}$/.test(body.paidDate) || !body.paymentReference.trim()) return Response.json({ error: "Paid date and bank confirmation reference are required." }, { status: 400 });
    const remittance = await db.prepare("SELECT id, status FROM remittance_obligations WHERE id = ? AND workspace_id = ? LIMIT 1").bind(body.remittanceId, WORKSPACE_ID).first<{ id: string; status: string }>();
    if (!remittance) return Response.json({ error: "The remittance obligation was not found." }, { status: 404 });
    if (remittance.status === "Paid") return Response.json({ ok: true, id: remittance.id, status: "Paid", idempotent: true });
    await db.batch([
      db.prepare("UPDATE remittance_obligations SET status = ?, paid_date = ?, payment_reference = ?, reminder_date = NULL WHERE id = ? AND workspace_id = ?").bind("Paid", body.paidDate, body.paymentReference.trim(), remittance.id, WORKSPACE_ID),
      db.prepare("INSERT INTO audit_events (id, workspace_id, occurred_at, actor_email, action, entity_type, entity_id, summary, payload_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(`AE-${crypto.randomUUID()}`, WORKSPACE_ID, occurredAt, actor.email, "remittance.marked_paid", "remittance", remittance.id, `CRA remittance marked paid with bank reference ${body.paymentReference.trim()}`, JSON.stringify({ paidDate: body.paidDate, paymentReference: body.paymentReference.trim() })),
    ]);
  ...
  if (body.action === "approve_draft") {
    ...
    const lineResult = await db.prepare("SELECT employee_id AS employeeId, employee_name AS employeeName, banked_overtime_earned_hundredths AS bankedOvertimeEarnedHundredths, banked_overtime_used_hundredths AS bankedOvertimeUsedHundredths, gross_cents AS grossCents, income_tax_cents AS incomeTaxCents, cpp_cents AS cppCents, cpp2_cents AS cpp2Cents, ei_cents AS eiCents, other_deductions_cents AS otherDeductionsCents, net_pay_cents AS netPayCents FROM pay_run_draft_lines WHERE draft_id = ? AND workspace_id = ? ORDER BY employee_name ASC").bind(draft.id, WORKSPACE_ID).all<{ employeeId: string; employeeName: string; bankedOvertimeEarnedHundredths: number; bankedOvertimeUsedHundredths: number; grossCents: number; incomeTaxCents: number; cppCents: number; cpp2Cents: number; eiCents: number; otherDeductionsCents: number; netPayCents: number }>();
    ...
    const remittanceAmountCents = remittanceLiabilityCents(lines);
    const account = await db.prepare("SELECT remitter_type AS remitterType FROM payroll_accounts WHERE id = ? AND workspace_id = ? LIMIT 1").bind(draft.payrollAccountId, WORKSPACE_ID).first<{ remitterType: string }>();
    ...
    await db.batch([
      ...
      ...lines.filter((line) => line.bankedOvertimeEarnedHundredths > 0).map((line) => db.prepare("INSERT INTO overtime_bank_entries (id, workspace_id, employee_id, pay_run_id, entry_type, transaction_date, hours_delta_hundredths, expires_on, source_reference, note, created_at, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(`OTB-${payRunId}-${line.employeeId}-EARN`, WORKSPACE_ID, line.employeeId, payRunId, "Earned", draft.periodEnd, line.bankedOvertimeEarnedHundredths, addUtcMonths(draft.periodEnd, 6), `pay-run:${payRunId}:${line.employeeId}:earned`, `Banked from pay run ${draft.runNumber}`, occurredAt, actor.email)),
      ...lines.filter((line) => line.bankedOvertimeUsedHundredths > 0).map((line) => db.prepare("INSERT INTO overtime_bank_entries (id, workspace_id, employee_id, pay_run_id, entry_type, transaction_date, hours_delta_hundredths, expires_on, source_reference, note, created_at, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(`OTB-${payRunId}-${line.employeeId}-USE`, WORKSPACE_ID, line.employeeId, payRunId, "Used", draft.payDate, -line.bankedOvertimeUsedHundredths, null, `pay-run:${payRunId}:${line.employeeId}:used`, `Paid time off in pay run ${draft.runNumber}`, occurredAt, actor.email)),
      db.prepare("INSERT INTO remittance_obligations (id, workspace_id, payroll_account_id, pay_run_id, period_end, due_date, liability_cents, status, reminder_date, paid_date, payment_reference, created_at, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(`REM-${payRunId}`, WORKSPACE_ID, draft.payrollAccountId, payRunId, draft.periodEnd, dueDate, remittanceAmountCents, "Due", null, null, null, occurredAt, actor.email),
