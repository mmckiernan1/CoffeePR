...
export default function PilotUatPage() {
  ...
  const [notice, setNotice] = useState("UAT state is local to this preview. Use fictional data only.");
  ...
  function addHire(event: FormEvent) {
    ...
    const id = `EMP-UAT-${String(employees.length + 1).padStart(2, "0")}`;
    ...
    setHireName("");
    setNotice(`${employee.name} was added as a UAT new hire.`);
  }
  ...
  function applyRateChange(event: FormEvent) {
    ...
    setNotice(`${employee.name}'s ${employee.payType === "Hourly" ? "hourly rate" : "annual salary"} change is ready for payroll review.`);
  }
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mt-6 grid gap-5 lg:grid-cols-3">
          ...
          <div className="rounded-[26px] border border-[#decdbd] bg-[#fffaf5] p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm leading-6 text-[#795f4f]">Test an effective payroll rate change without rebuilding the employee.</p>
        ...
        <section className="mt-6 rounded-[26px] border border-[#decdbd] bg-[#fffaf5] p-6 shadow-sm">
          ...
          <div className="mt-6 flex flex-wrap gap-3"><button onClick={() => router.push("/guided-payroll")} className="rounded-xl bg-[#5a321f] px-5 py-3 font-semibold text-white">Continue to guided payroll</button><button onClick={() => { setEmployees(starterEmployees); setTimesheets(starterTime); setReady(false); setNotice("UAT sample data was reset."); }} className="rounded-xl border border-[#d6c6b8] px-5 py-3 font-semibold">Reset UAT data</button></div>
        </section>
      </div>
    </main>
  );
